...
static const char* get_time_str() {
    static thread_local char buf[64];  // 每个线程独立，避免竞争
...
// 修正：这个函数找的是 mRawNameId 的偏移量，不是字符串值
...
// 修正：这个函数找的是 mNamespace 的偏移量，不是字符串值  
